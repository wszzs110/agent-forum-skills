...
async function runCli(args: string[]) {
  const command = new Deno.Command(cli, { args: [...(cliScript ? [cliScript] : []), "--json", ...args], stdout: "piped", stderr: "piped" });
...
function startCli(args: string[]): Promise<boolean> {
  ...
  try {
    ...
    const child = new Deno.Command(cli, { args: commandArgs, stdout: "piped", stderr: "piped", clearEnv: true, env: childEnv }).spawn();
...
const server = Deno.serve(async (request) => {
  ...
  if (url.pathname === "/" && request.method === "GET") return new Response(page(url.origin, token, demoExtremeCounts), { headers: { ...corsHeaders, "content-type": "text/html; charset=utf-8" } });
  ...
  try {
    ...
    if (url.pathname === "/window" && request.method === "POST") { const value = await request.json() as { mode?: number }; if (value.mode !== 0 && value.mode !== 1 && value.mode !== 3) return new Response("invalid window mode", { status: 400, headers: corsHeaders }); desktopWindow?.setSize(670, value.mode === 0 ? 124 : value.mode === 1 ? 62 : 344); return jsonResponse({ mode: value.mode }); }
    if (url.pathname === "/top" && request.method === "POST") { const value = await request.json() as { enabled?: boolean }; if (typeof value.enabled !== "boolean") return new Response("invalid always-on-top request", { status: 400, headers: corsHeaders }); desktopAlwaysOnTop = value.enabled; applyAlwaysOnTop(); return jsonResponse({ enabled: desktopAlwaysOnTop }); }
    ...
    if (url.pathname === "/viewer" && request.method === "POST") { const value = await request.json() as { forumAlias?: string; roomId?: string }; if (typeof value.forumAlias !== "string" || typeof value.roomId !== "string") return new Response("invalid Viewer request", { status: 400, headers: corsHeaders }); if (!(await startCli(["viewer", "open", "--forum", value.forumAlias, "--room", value.roomId, "--home", home]))) return new Response("Viewer could not be opened; inspect the Dashboard helper diagnostics.", { status: 500, headers: corsHeaders }); return jsonResponse({ opening: true, forumAlias: value.forumAlias, roomId: value.roomId }); }
...
function applyAlwaysOnTop() {
  ...
  // Linux CEF/Wayland 可能在窗口映射前忽略构造参数；每次映射和用户切换时重新应用。
