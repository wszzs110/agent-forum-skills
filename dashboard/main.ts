...
async function attachLease(input: ClientInput, resetView = false) {
...
function page(apiBase: string, pageToken: string, demoExtremeCounts = false, topSupported = true, initialLanguage: "en" | "zh" = "en"): string {
  return `<!doctype html><meta charset="utf-8"><meta name="viewport" content="width=device-width"><title>Agent Forum Dashboard</title>
<style>
:root{color-scheme:light;font:12px -apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif;background:#f5f5f7;color:#1d1d1f}*{box-sizing:border-box}body{margin:0;min-width:620px;overflow:hidden;background:linear-gradient(135deg,#f8fafc,#edf2f9)}.bar{position:relative;height:124px;padding:8px 10px;background:rgba(246,248,252,.92);border:1px solid rgba(255,255,255,.9);border-radius:14px;box-shadow:0 10px 28px rgba(31,45,61,.16);backdrop-filter:blur(24px)}.teams{height:42px;display:flex;align-items:center;gap:7px;padding:0 2px;-webkit-app-region:drag}.brand{display:flex;align-items:center;min-width:27px}.brandmark{display:block;width:27px;height:27px;filter:drop-shadow(0 3px 6px rgba(19,31,52,.22))}.team-tabs{display:flex;flex:1;min-width:0;align-items:center;gap:5px;overflow:hidden;padding-right:185px}.team{height:34px;min-width:0;display:flex;align-items:center;overflow:hidden;border:1px solid transparent;-webkit-app-region:no-drag;border-radius:10px;background:rgba(222,227,235,.68);color:#51545b;padding:0 10px;cursor:pointer;white-space:nowrap;font-weight:650;transition:.16s}.team-label{min-width:0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.many-teams .team{flex:1 1 0;padding:0 6px}.many-teams .team.active{flex-grow:3}.many-teams .team .dot{margin-right:4px}.team:hover{background:#e4e9f2}.team.active{background:#fff;color:#202124;border-color:rgba(73,100,150,.13);box-shadow:0 2px 7px rgba(28,39,56,.1)}.team .dot{display:inline-block;flex:0 0 auto;width:7px;height:7px;border-radius:50%;margin-right:6px;vertical-align:1px}.rooms{height:64px;display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:7px;align-items:stretch;padding:7px 2px 0;overflow:hidden}.room{position:relative;display:flex;align-items:center;gap:9px;min-width:0;max-width:none;-webkit-app-region:no-drag;overflow:hidden;border:1px solid rgba(80,95,118,.14);border-radius:12px;background:rgba(255,255,255,.82);box-shadow:0 2px 7px rgba(37,50,71,.07);padding:8px 9px;cursor:pointer;transition:.16s}.room:hover{transform:translateY(-1px);box-shadow:0 5px 14px rgba(37,50,71,.12)}.room.selected{border-color:#4f80e8;box-shadow:inset 4px 0 #4f80e8,0 0 0 2px rgba(79,128,232,.16),0 4px 12px rgba(37,50,71,.12)}.room.deprecated{background:rgba(239,241,244,.72);border-color:rgba(112,120,133,.14);box-shadow:none;filter:grayscale(.72);opacity:.64}.room.deprecated:hover{transform:none;box-shadow:none}.room.deprecated.selected{border-color:rgba(112,120,133,.28);box-shadow:inset 4px 0 #9ca3ad,0 2px 7px rgba(37,50,71,.06)}.deprecated-badge{display:inline-block;margin-top:3px;padding:1px 4px;border-radius:3px;background:rgba(112,120,133,.14);color:#68717d;font-size:8px;font-weight:750;letter-spacing:.35px;text-transform:uppercase}.room-name{min-width:0;flex:1;overflow:hidden;font-size:12px;font-weight:680;letter-spacing:-.1px}.title-viewport{min-width:0;overflow:hidden;white-space:nowrap}.title-track{display:inline-block;width:max-content;white-space:nowrap;padding-right:26px;will-change:transform}.agent{display:block;margin-top:3px;color:#4c9a70;font-size:9px;font-weight:700}.metrics{margin-left:auto;display:flex;flex-direction:column;gap:2px;align-items:flex-end;font-variant-numeric:tabular-nums;min-width:38px}.metric{display:flex;align-items:center;justify-content:flex-end;gap:3px;min-width:38px;white-space:nowrap;color:#7d838e;font-size:10px;font-weight:700}.metric svg{width:11px;height:11px;fill:none;stroke:currentColor;stroke-width:1.8;stroke-linecap:round;stroke-linejoin:round}.metric.related{color:#e36b7e}.metric.broadcast{color:#d59627}.metric.other{color:#8a94a4}.right{position:absolute;right:10px;top:9px;z-index:4;display:flex;gap:5px;align-items:center;-webkit-app-region:no-drag}.button{width:32px;height:32px;display:grid;place-items:center;border:1px solid rgba(76,91,116,.16);background:rgba(255,255,255,.72);color:#5d6470;border-radius:10px;padding:0;cursor:pointer;font:650 11px inherit;transition:.16s}.button svg{width:16px;height:16px;fill:none;stroke:currentColor;stroke-width:1.8;stroke-linecap:round;stroke-linejoin:round}.button:hover{background:#fff;box-shadow:0 2px 6px rgba(37,50,71,.1)}.button:disabled{cursor:not-allowed;opacity:.42}.button.on{background:#e9f0ff;border-color:#b5cdfb;color:#356bd6}.button.close{color:#d35c69}.button.on{box-shadow:inset 0 0 0 1px rgba(53,107,214,.08)}.button.syncing svg{animation:dashboard-spin .9s linear 1}@keyframes dashboard-spin{to{transform:rotate(360deg)}}.error{padding:14px;color:#c45062}.compact{height:62px;padding:7px 10px}.compact .teams{height:0;padding:0}.compact .brand,.compact .team-tabs{display:none}.compact .right{display:flex;position:absolute;right:10px;top:15px;z-index:2;gap:5px}.compact .rooms{height:48px;padding:5px 194px 0 2px;grid-template-columns:1fr}.compact .room{height:40px;border-radius:10px;padding:4px 7px}.compact .room-name{font-size:11px}.compact .title-track{padding-right:18px}.compact .metrics{flex-direction:row;gap:4px;min-width:0}.compact .metric{font-size:9px}.compact .agent{display:none}.expanded{height:344px}.expanded .rooms{height:264px;margin-right:38px;grid-template-columns:repeat(3,minmax(0,1fr));grid-auto-rows:64px;align-content:start;overflow-y:auto;padding:7px 0;scrollbar-width:none}.expanded .rooms::-webkit-scrollbar{display:none}.room-scrollbar{display:none}.expanded .room-scrollbar{display:block;position:absolute;right:5px;top:49px;bottom:39px;width:28px;border:1px solid rgba(76,91,116,.16);border-radius:8px;background:rgba(255,255,255,.5);overflow:hidden;-webkit-app-region:no-drag}.room-scroll-thumb{position:absolute;left:2px;top:2px;width:22px;min-height:28px;border:1px solid #b5cdfb;border-radius:6px;background:#e3ecff;box-shadow:0 1px 3px rgba(53,107,214,.12);cursor:grab}.room-scroll-thumb:active{cursor:grabbing;background:#d5e3ff}.room-more{position:absolute;right:5px;bottom:5px;z-index:3;width:28px;height:24px;border-radius:8px}.room-more svg{width:14px;height:14px}.bar.has-more:not(.expanded) .rooms{padding-right:38px}.rail{height:10px;padding:0;border-radius:0}.rail .teams{height:8px;padding:0;gap:2px}.rail .brand,.rail .right{display:none}.rail .team-tabs{width:100%;height:8px;gap:2px}.rail .team{height:8px;flex:1;padding:0;font-size:0;border-radius:0}.rail .team.active{box-shadow:none;border-color:rgba(255,255,255,.8)}.rail .rooms{display:none}.status{color:#8b929e;font-size:10px}.notice{position:fixed;z-index:20;right:10px;bottom:8px;display:flex;max-width:min(460px,calc(100vw - 20px));align-items:center;gap:8px;padding:7px 8px 7px 10px;border:1px solid #efb7bf;border-radius:9px;background:rgba(255,247,248,.97);box-shadow:0 5px 16px rgba(80,34,42,.18);color:#9c3545;font-size:11px;line-height:1.35;word-break:break-word;-webkit-app-region:no-drag}.notice button{flex:0 0 auto;width:20px;height:20px;border:0;border-radius:5px;background:transparent;color:inherit;cursor:pointer;font-size:16px;line-height:1}.notice button:hover{background:#f8dfe3}
#room-panel{display:none;max-height:520px;overflow-y:auto;padding:10px 12px 14px;background:linear-gradient(180deg,rgba(248,250,254,.96),rgba(237,242,249,.98));border:1px solid rgba(255,255,255,.85);border-top:none;border-radius:0 0 14px 14px;box-shadow:0 12px 32px rgba(31,45,61,.14);backdrop-filter:blur(20px);font:12px -apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif;color:#1d1d1f;scrollbar-width:thin;scrollbar-color:#c8cdd6 transparent}#room-panel::-webkit-scrollbar{width:6px}#room-panel::-webkit-scrollbar-thumb{background:#c8cdd6;border-radius:3px}.room-view{height:100vh;padding:0;overflow:hidden;border-radius:14px;background:linear-gradient(145deg,#f8fafc,#e9eff8);border:1px solid rgba(255,255,255,.92);box-shadow:0 10px 28px rgba(31,45,61,.16)}.room-view .rv-toolbar{height:52px;display:flex;align-items:center;padding:0 12px 0 15px;border-bottom:1px solid rgba(80,95,118,.12);background:rgba(250,252,255,.86);backdrop-filter:blur(20px);-webkit-app-region:drag}.room-view .rv-context{display:flex;flex:1;align-items:center;gap:9px;min-width:0}.room-view .rv-actions{display:flex;align-items:center;gap:5px;flex-shrink:0;-webkit-app-region:no-drag}.room-view .rv-mark{display:grid;place-items:center;width:27px;height:27px;border-radius:8px;background:#0c101a;color:#70e1d0;font-size:15px;font-weight:800}.room-view .rv-title{min-width:0;font-size:13px;font-weight:750;letter-spacing:-.15px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.room-view .rv-subtitle{margin-top:1px;font-size:9px;color:#7a7e87}.room-view .rv-toolbar .button{-webkit-app-region:no-drag}.room-view #room-panel{display:block;height:calc(100vh - 52px);max-height:none;overflow-y:auto;padding:10px 14px 16px;border:0;border-radius:0;background:transparent;box-shadow:none}.room-view .rp-header{padding-top:0}.room-view .rp-section{margin-top:12px}.rp-loading{padding:24px;text-align:center;color:#8b929e}.rp-empty{padding:24px;text-align:center;color:#8b929e}.rp-header{padding:8px 0 6px;border-bottom:1px solid rgba(80,95,118,.1)}.rp-room-name{font-size:16px;font-weight:750;letter-spacing:-.3px;color:#1a1d23}.rp-room-desc{font-size:11px;color:#6b7280;margin-top:2px}.rp-info{font-size:10px;color:#8b929e;margin-top:4px;font-variant-numeric:tabular-nums}.rp-section{margin-top:10px}.rp-section-title{font-size:10px;font-weight:700;text-transform:uppercase;letter-spacing:.6px;color:#8b929e;margin-bottom:5px}.rp-thread{border:1px solid rgba(80,95,118,.12);border-radius:8px;background:rgba(255,255,255,.7);margin-bottom:4px;overflow:hidden;transition:.15s}.rp-thread:hover{border-color:rgba(79,128,232,.25)}.rp-thread.open{border-color:rgba(79,128,232,.3);box-shadow:0 2px 8px rgba(79,128,232,.08)}.rp-thread-head{display:flex;align-items:center;gap:6px;padding:6px 8px;cursor:pointer;-webkit-app-region:no-drag}.rp-thread-kind{font-size:9px;font-weight:700;text-transform:uppercase;letter-spacing:.4px;padding:2px 6px;border-radius:4px;background:rgba(79,128,232,.1);color:#4f80e8;flex-shrink:0}.rp-thread-status{font-size:8px;font-weight:750;text-transform:uppercase;letter-spacing:.4px;padding:2px 5px;border-radius:4px;flex-shrink:0}.rp-thread-status.closed{background:rgba(112,120,133,.12);color:#68717d}.rp-thread.closed{background:rgba(245,246,248,.72);border-color:rgba(112,120,133,.14)}.rp-thread.closed .rp-thread-title{color:#68717d}.rp-thread-title{font-size:12px;font-weight:650;min-width:0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;flex:1}.rp-thread-meta{font-size:9px;color:#8b929e;white-space:nowrap;flex-shrink:0;font-variant-numeric:tabular-nums}.rp-messages{padding:4px 8px 8px;border-top:1px solid rgba(80,95,118,.08);max-height:280px;overflow-y:auto}.rp-msg{display:flex;gap:6px;padding:5px 0;border-bottom:1px solid rgba(80,95,118,.06)}.rp-msg:last-child{border-bottom:none}.rp-msg.unread-selected{background:#fff8dd;box-shadow:inset 3px 0 #f59e0b;border-radius:5px}.rp-msg.unread-focus{animation:rp-unread-focus .9s ease-out}@keyframes rp-unread-focus{35%{background:#fff7d6;box-shadow:0 0 0 3px #fde68a;border-radius:5px}}.rp-avatar{width:22px;height:22px;border-radius:50%;display:flex;align-items:center;justify-content:center;font-size:9px;font-weight:700;color:#fff;flex-shrink:0}.rp-msg-body{min-width:0;flex:1}.rp-msg-author{font-weight:650;font-size:11px}.rp-msg-type{font-size:9px;color:#8b929e;text-transform:uppercase}.rp-msg-time{font-size:9px;color:#a0a6b0;margin-left:4px}.rp-receipt{display:inline-flex;margin-left:4px;padding:1px 5px;border-radius:999px;font-size:8px;font-weight:750}.rp-receipt.read{background:#dcfce7;color:#16803d}.rp-receipt.unread{background:#fffbeb;color:#b96808}.rp-receipt.published{background:#dbeafe;color:#356bd6}.rp-msg-text{font-size:11px;color:#3d4250;margin-top:2px;line-height:1.45;word-break:break-word}.rp-msg-text p{margin:0 0 6px}.rp-msg-text p:last-child{margin-bottom:0}.rp-msg-text h3,.rp-msg-text h4,.rp-msg-text h5,.rp-msg-text h6{margin:8px 0 4px;font-size:12px;line-height:1.3}.rp-msg-text ul,.rp-msg-text ol{margin:4px 0;padding-left:18px}.rp-msg-text li{margin:2px 0}.rp-msg-text .inline-code{padding:1px 3px;border-radius:3px;background:rgba(79,128,232,.1);font:10px ui-monospace,SFMono-Regular,Consolas,monospace}.rp-msg-text .code-block{max-height:180px;overflow:auto;margin:5px 0;padding:6px;border-radius:5px;background:#202633;color:#e8edf7;font:10px/1.4 ui-monospace,SFMono-Regular,Consolas,monospace;white-space:pre-wrap}.rp-msg-text .md-quote{margin:5px 0;padding:3px 7px;border-left:3px solid #9bb8ef;background:rgba(79,128,232,.06);color:#626b78}.rp-msg-text .md-table-wrap{max-width:100%;overflow:auto;margin:5px 0}.rp-msg-text .md-table{border-collapse:collapse;font-size:10px}.rp-msg-text .md-table th,.rp-msg-text .md-table td{padding:3px 5px;border:1px solid rgba(80,95,118,.15);text-align:left}.rp-msg-text a{color:#356bd6;text-decoration:none}.rp-member{display:flex;align-items:center;gap:6px;padding:4px 0;border-bottom:1px solid rgba(80,95,118,.06)}.rp-member:last-child{border-bottom:none}.rp-member.idle{opacity:.45}.rp-member-name{font-size:11px;font-weight:650;min-width:60px;max-width:90px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.rp-member-role{font-size:9px;color:#8b929e;min-width:48px;max-width:70px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.rp-bar-track{flex:1;height:6px;background:rgba(80,95,118,.08);border-radius:3px;overflow:hidden;min-width:40px}.rp-bar-fill{height:100%;background:linear-gradient(90deg,#4f80e8,#68c6e3);border-radius:3px;transition:width .4s ease-out}.rp-member-count{font-size:9px;color:#6b7280;font-variant-numeric:tabular-nums;min-width:42px;text-align:right}.rp-member-time{font-size:9px;color:#a0a6b0;min-width:48px;text-align:right}.rp-pulse{width:6px;height:6px;border-radius:50%;background:#4c9a70;animation:rp-breathe 2s ease-in-out infinite;flex-shrink:0}@keyframes rp-breathe{0%,100%{opacity:.4;transform:scale(.8)}50%{opacity:1;transform:scale(1.1)}}
</style><div id="app" class="bar"><div class="error">Loading Dashboard…</div></div>
<script>
let selected,selectedRoomId,mode=0,revision=-1,lastData,alwaysOnTop=${JSON.stringify(alwaysOnTopSupported)},pollingSequence=-1,roomPanelExpanded=false,roomPanelOpen=false,roomPanelData=null,roomPanelLoading=false,roomPanelRevision=-1,roomPanelRefresh=null,expandedThreadId=null,roomUnreadId=null;const topSupported=${JSON.stringify(topSupported)},initialLanguage=${JSON.stringify(initialLanguage)},roomOrders=new Map(),selectedRoomIds=new Map(),teamSignatures=new Map(),app=document.getElementById('app'),apiBase=${JSON.stringify(apiBase)},token=${JSON.stringify(pageToken)},demoExtreme=${JSON.stringify(demoExtremeCounts)};
...
function messageBody(message){return typeof message.bodyHtml==='string'?message.bodyHtml:esc(message.body)}
let roomLanguage=typeof initialLanguage==='string'?initialLanguage:'en';const roomText=(en,zh)=>roomLanguage==='zh'?zh:en;const roomLabel=(value,kind)=>{const labels={role:{backend:'后端',frontend:'前端',fullstack:'全栈',mobile:'移动端',qa:'测试',devops:'运维',security:'安全',designer:'设计',product:'产品'},type:{discussion:'讨论',question:'问题',answer:'答复',proposal:'提案',decision:'决策',change:'变更',blocker:'阻塞',review:'评审',status:'状态','test-result':'测试结果',acknowledgement:'确认',objection:'异议',correction:'纠正','thread-closed':'主题已关闭','thread-reopened':'主题已重开'}};return roomLanguage==='zh'?(labels[kind]?.[value]||value):value};function receiptBadge(receipt){if(!receipt)return'';const published=receipt.publishedBy?.length||0,read=receipt.readBy?.length||0,unread=receipt.unreadBy?.length||0,total=read+unread;if(total)return'<span class="rp-receipt '+(unread?'unread':'read')+'">'+(published?(roomText('AI published','AI 发布')+' · '):'')+(total===1?(read?roomText('AI read','AI 已读'):roomText('AI unread','AI 未读')):read+'/'+total+' '+roomText('local AIs read','个本机 AI 已读'))+'</span>';return published?'<span class="rp-receipt published">'+roomText('AI published','AI 发布')+'</span>':''}
function renderRoomPanel(data){if(!data)return'<div class="rp-empty">'+roomText('No room data.','没有 Room 数据。')+'</div>';const maxMsg=Math.max(1,...data.members.map(m=>m.messageCount));return'<div class="rp-header"><div class="rp-room-name">'+esc(data.room.title)+'</div><div class="rp-room-desc">'+esc(data.room.description)+'</div><div class="rp-info">'+esc(data.forum.name)+' · '+esc(data.forum.dataBranch)+' · '+timeAgo(data.syncedAt)+' · '+data.stats.threadCount+' '+roomText('threads','个主题')+' · '+data.stats.messageCount+' '+roomText('messages','条消息')+' · '+data.stats.memberCount+' '+roomText('members','名成员')+'</div></div><div class="rp-section"><div class="rp-section-title">'+roomText('Threads','主题')+'</div>'+data.threads.map(t=>'<div class="rp-thread'+(expandedThreadId===t.id?' open':'')+(t.status==='closed'?' closed':'')+'" data-thread="'+esc(t.id)+'"><div class="rp-thread-head"><span class="rp-thread-kind">'+esc(roomLabel(t.kind,'type'))+'</span>'+(t.status==='closed'?'<span class="rp-thread-status closed">'+roomText('Closed','已关闭')+'</span>':'')+'<span class="rp-thread-title">'+esc(t.title)+'</span><span class="rp-thread-meta">'+esc(t.authorName)+' · '+t.replyCount+' '+roomText('replies','条回复')+' · '+timeAgo(t.lastActivityAt)+'</span></div>'+(expandedThreadId===t.id?'<div class="rp-messages">'+t.messages.map(m=>'<div class="rp-msg'+(m.localReceipt?.unreadBy?.length?' ai-unread':'')+'" data-message="'+esc(m.id)+'" style="margin-left:'+(m.replyTo?24:0)+'px"><span class="rp-avatar" style="background:'+memberColor(m.authorId)+'">'+esc(memberInitials(m.authorName))+'</span><div class="rp-msg-body"><span class="rp-msg-author">'+esc(m.authorName)+'</span> <span class="rp-msg-type">'+esc(roomLabel(m.type,'type'))+'</span> <span class="rp-msg-time">'+timeAgo(m.createdAt)+'</span>'+receiptBadge(m.localReceipt)+'<div class="rp-msg-text">'+messageBody(m)+'</div></div></div>').join('')+'</div>':'')+'</div>').join('')+'</div><div class="rp-section"><div class="rp-section-title">'+roomText('Members · by activity','成员 · 按活跃度')+'</div>'+data.members.map(m=>'<div class="rp-member'+(m.messageCount===0?' idle':'')+'"><span class="rp-avatar" style="background:'+memberColor(m.id)+'">'+esc(memberInitials(m.displayName))+'</span><span class="rp-member-name">'+esc(m.displayName)+'</span><span class="rp-member-role">'+esc(roomLabel(m.role,'role'))+'</span><div class="rp-bar-track"><div class="rp-bar-fill" style="width:'+Math.round(m.messageCount/maxMsg*100)+'%"></div></div><span class="rp-member-count">'+m.messageCount+' '+roomText('msgs','条消息')+'</span><span class="rp-member-time">'+(m.lastMessageAt?timeAgo(m.lastMessageAt):roomText('never','从未'))+'</span>'+(isRecent(m.lastMessageAt)?'<span class="rp-pulse"></span>':'')+'</div>').join('')+'</div>'}
...
function renderRoomView(team){const roomId=selectedRoomIds.get(team.forumId)||selectedRoomId||team.rooms[0]?.roomId;const selectedRoom=team.rooms.find(r=>r.roomId===roomId)||team.rooms[0];app.className='room-view';app.innerHTML='<div class="rv-toolbar"><div class="rv-context"><span class="rv-mark">#</span><div><div class="rv-title">'+esc(roomPanelData?.room?.title||selectedRoom?.title||'Room')+'</div><div class="rv-subtitle">'+esc(team.forumAlias)+'</div></div></div><div class="rv-actions"><button class="button" id="previous-unread" aria-label="previous unread">‹</button><button class="button" id="next-unread" aria-label="next unread">›</button><button class="button" id="room-language" aria-label="toggle language">'+(roomLanguage==='zh'?'EN':'中')+'</button><button class="button on" id="viewer" aria-label="close room page">'+icon('eyeClosed')+'</button></div></div><div id="room-panel">'+(roomPanelLoading?'<div class="rp-loading">Loading room…</div>':renderRoomPanel(roomPanelData))+'</div>';app.querySelector('#viewer').onclick=()=>{roomPanelOpen=false;void toggleRoomPanel(team)};app.querySelector('#room-language').onclick=async()=>{try{const result=await api('/language',{language:roomLanguage==='en'?'zh':'en'});roomLanguage=result.language;renderRoomView(team)}catch(e){showNotice('Language preference failed: '+e.message)}};const showUnread=(id,scroll=false)=>{const entry=app.querySelector('.rp-msg[data-message="'+id+'"]');app.querySelectorAll('.rp-msg.unread-selected').forEach(item=>item.classList.remove('unread-selected'));if(!entry)return;entry.classList.add('unread-selected');if(scroll){entry.scrollIntoView({behavior:'smooth',block:'center'});entry.classList.add('unread-focus');setTimeout(()=>entry.classList.remove('unread-focus'),900)}};const moveUnread=direction=>{const entries=(roomPanelData?.threads||[]).flatMap(thread=>thread.messages.filter(message=>message.localReceipt?.unreadBy?.length).map(message=>({id:message.id,threadId:thread.id})));if(!entries.length)return;const current=entries.findIndex(entry=>entry.id===roomUnreadId),next=entries[current<0?0:(current+direction+entries.length)%entries.length];roomUnreadId=next.id;expandedThreadId=next.threadId;renderRoomView(team);requestAnimationFrame(()=>showUnread(next.id,true))};if(roomUnreadId)showUnread(roomUnreadId);app.querySelector('#previous-unread').onclick=()=>moveUnread(-1);app.querySelector('#next-unread').onclick=()=>moveUnread(1);bindRoomThreads(team)}
async function toggleRoomPanel(team){if(!roomPanelOpen){roomPanelData=null;roomPanelLoading=false;roomPanelRevision=-1;expandedThreadId=null;roomUnreadId=null;await api('/window',{mode,panelHeight:0});render(lastData);return}const forumAlias=team.forumAlias,roomId=selectedRoomIds.get(team.forumId)||selectedRoomId||team.rooms[0]?.roomId;if(!roomId){roomPanelOpen=false;showNotice('No room selected.');render(lastData);return}roomPanelLoading=true;roomPanelData=null;roomPanelRevision=-1;expandedThreadId=null;roomUnreadId=null;await api('/window',{mode,panelHeight:430});render(lastData);try{roomPanelData=await api('/room-panel',{forumAlias,roomId,identityIds:team.identityIds||[]});roomPanelLoading=false;roomPanelRevision=revision;render(lastData)}catch(e){roomPanelOpen=false;roomPanelLoading=false;roomPanelData=null;await api('/window',{mode,panelHeight:0});render(lastData);showNotice('Room page failed: '+e.message)}}
...
function refreshOpenRoomPanel(team,nextRevision){if(roomPanelRefresh)return;const forumAlias=team.forumAlias,roomId=selectedRoomIds.get(team.forumId)||selectedRoomId||team.rooms[0]?.roomId;if(!roomId)return;roomPanelRefresh=api('/room-panel',{forumAlias,roomId,identityIds:team.identityIds||[]}).then(data=>{const currentTeam=lastData?.teams?.find(item=>item.forumId===team.forumId),currentRoom=currentTeam&&(selectedRoomIds.get(currentTeam.forumId)||selectedRoomId||currentTeam.rooms[0]?.roomId);if(roomPanelOpen&&currentRoom===roomId){roomPanelData=data;roomPanelRevision=nextRevision;render(lastData)}}).catch(e=>{roomPanelRevision=nextRevision;showNotice('Room page refresh failed: '+e.message)}).finally(()=>{roomPanelRefresh=null})}
async function refresh(force=false){try{const data=await api('/snapshot'),changed=force||data.revision!==revision;if(changed)render(data,true);const team=data.teams?.find(item=>item.forumId===selected);if(roomPanelOpen&&team&&roomPanelRevision!==data.revision)refreshOpenRoomPanel(team,data.revision)}catch(e){showNotice('Dashboard refresh failed: '+e.message)}void refreshPollingStatus()}
...
let uiLanguage: "en" | "zh" = "en";
try {
  const preference = await runCli(["preference", "language"]) as { language?: unknown };
  if (preference.language === "zh") uiLanguage = "zh";
} catch { /* preference failures must not prevent Dashboard startup */ }
...
const server = Deno.serve(async (request) => {
  ...
  if (url.pathname === "/" && request.method === "GET") return new Response(page(url.origin, token, demoExtremeCounts, alwaysOnTopSupported, uiLanguage), { headers: { ...corsHeaders, "content-type": "text/html; charset=utf-8" } });
  ...
  try {
    ...
    if (url.pathname === "/language" && request.method === "POST") { const value = await request.json() as { language?: unknown }; if (value.language !== "en" && value.language !== "zh") return new Response("invalid language", { status: 400, headers: corsHeaders }); await runCli(["preference", "language", "--value", value.language]); uiLanguage = value.language; return jsonResponse({ language: uiLanguage }); }
    ...
    if (url.pathname === "/detach" && request.method === "POST") { const value = await request.json() as { clientId?: string }; if (typeof value.clientId !== "string") return new Response("invalid client", { status: 400, headers: corsHeaders }); clients.delete(value.clientId); return jsonResponse({ detached: true }); }
    ...
    if (url.pathname === "/room-panel" && request.method === "POST") { const value = await request.json() as { forumAlias?: string; roomId?: string; identityIds?: unknown }; if (typeof value.forumAlias !== "string" || typeof value.roomId !== "string" || (value.identityIds !== undefined && (!Array.isArray(value.identityIds) || !value.identityIds.every((id) => typeof id === "string")))) return new Response("invalid room panel request", { status: 400, headers: corsHeaders }); const identities = (value.identityIds ?? []) as string[]; const data = await runCli(["viewer", "data", "--forum", value.forumAlias, "--room", value.roomId, ...identities.flatMap((id) => ["--identity", id])]); return jsonResponse(data); }
...
const leaseTimer = setInterval(async () => {
  ...
  try {
    const status = await runCli(["dashboard", "lease-status"]) as { clients?: Array<{ clientId: string }> };
