...
const icon=name=>({eye:'<svg viewBox="0 0 24 24"><path d="M2.5 12s3.5-5 9.5-5 9.5 5 9.5 5-3.5 5-9.5 5-9.5-5-9.5-5z"/><circle cx="12" cy="12" r="2.5"/></svg>',polling:'<svg viewBox="0 0 24 24"><path d="M5 8a8 8 0 0 1 13-2l2 2M19 16a8 8 0 0 1-13 2l-2-2M20 8h-4M4 16h4"/></svg>',top:'<svg viewBox="0 0 24 24"><path d="M8 3h8l-1 5 3 3v2H6v-2l3-3-1-5M12 13v8"/></svg>',up:'<svg viewBox="0 0 24 24"><path d="m6 14 6-6 6 6"/></svg>',down:'<svg viewBox="0 0 24 24"><path d="m6 10 6 6 6-6"/></svg>',close:'<svg viewBox="0 0 24 24"><path d="m7 7 10 10M17 7 7 17"/></svg>'}[name]||'');
...
const server = Deno.serve(async (request) => {
  ...
  try {
    ...
    if (url.pathname === "/top" && request.method === "POST") { const value = await request.json() as { enabled?: boolean }; if (typeof value.enabled !== "boolean") return new Response("invalid always-on-top request", { status: 400, headers: corsHeaders }); desktopAlwaysOnTop = value.enabled; desktopWindow?.setAlwaysOnTop(desktopAlwaysOnTop); return jsonResponse({ enabled: desktopAlwaysOnTop }); }
    ...
    if (url.pathname === "/viewer" && request.method === "POST") { const value = await request.json() as { forumAlias?: string; roomId?: string }; if (typeof value.forumAlias !== "string" || typeof value.roomId !== "string") return new Response("invalid Viewer request", { status: 400, headers: corsHeaders }); void startCli(["viewer", "launch", "--forum", value.forumAlias, "--room", value.roomId, "--home", home]); return jsonResponse({ opening: true, forumAlias: value.forumAlias, roomId: value.roomId }); }
...
setTimeout(() => { startupWindow.close(); desktopWindow?.show(); desktopWindow?.focus(); configureWindowsWindow(); for (let attempt = 1; attempt < 8; attempt += 1) setTimeout(configureWindowsWindow, attempt * 250); }, 500);
