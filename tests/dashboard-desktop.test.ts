...
test("Dashboard 最后一个 Agent 离开后等待用户手动关闭", async () => {
  ...
  assert.match(page, /title="Related"><span class="binding-slot'\+\(r\.bindings\?\.length\?'':' empty'\)\+'/u, "the binding slot precedes the Related unread indicator");
  assert.match(page, /\.compact \.metrics\{display:grid;grid-template-columns:repeat\(4,22px\);column-gap:2px/u, "collapsed notification icons use four equal grid columns");
