...
test("Dashboard 最后一个 Agent 离开后等待用户手动关闭", async () => {
  ...
  assert.match(page, /\.room-name\{position:relative;[^}]*overflow:visible/u, "Room title keeps the same vertical anchor when Active is present");
  assert.match(page, /\.room-status-line\{position:absolute;left:0;top:calc\(100% \+ 2px\)/u, "Active status is positioned below the fixed Room title anchor");
  assert.match(page, /function bindingTooltip\(binding\)/u, "bound workspace tooltip is generated from branch and directory data");
  assert.match(page, /function renderBindingMarker\(bindings\)/u, "bound workspace marker is rendered only for bound Rooms");
  assert.match(page, /class="binding-marker"[^>]*title=/u, "bound workspace marker exposes its tooltip through the native title");
  assert.match(page, /class="binding-marker"[^>]*><svg viewBox="0 0 16 16"/u, "bound workspace marker uses a compact vector chain icon");
  ...
  const sendMarkerPos = page.indexOf("renderSendModeMarker(r.sendMode||'auto')");
  const bindingSlotPos = page.indexOf('class="binding-slot');
  assert.ok(sendSlotPos >= 0 && sendMarkerPos > sendSlotPos && bindingSlotPos > sendMarkerPos, "the send-mode slot precedes the binding chain marker");
  assert.match(page, /\+':'\+room\.sendMode\)\.join\('\|'\)/u, "Room signature includes send mode for reconciliation");
  assert.match(page, /class="compact-active-dot"/u, "collapsed Active Rooms expose a compact green-dot marker");
...
test("Viewer 打开失败只显示可关闭提示，不替换 Dashboard Bar", async () => {
  ...
  assert.match(source, /roomSearchState=\{text:''[^;]*tags:/u, "Dashboard Room page keeps a unified search state");
  assert.match(source, /buildRoomTagChips=/u, "Dashboard Room page renders tag chips from panel data");
  assert.match(source, /moveRoomResult=direction=>/u, "Dashboard previous/next navigate the result set");
  assert.match(source, /list\.findIndex\(r=>r\.id===roomSearchState\.selection\)/u, "Dashboard first navigation selects the first result as the baseline");
