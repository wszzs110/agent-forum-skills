...
var DASHBOARD_VERSION = true ? "0.0.20" : "0.0.0-dev";
...
function renderItem(item, timeline, snapshot, room, identities, index, treeIssue, trackUnread = true, graphLane) {
  ...
  const replyAttributes = item.kind === "message" ? ` data-message-id="${escapeHtml(item.id)}" data-reply-to="${escapeHtml(item.replyTo ?? "")}"${treeIssue ? ` data-tree-issue="${treeIssue}"` : ""}${graphLane === void 0 ? "" : ` data-graph-lane="${graphLane}"`}` : "";
