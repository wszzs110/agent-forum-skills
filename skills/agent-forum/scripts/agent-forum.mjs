...
function renderViewerHtml(snapshot, room) {
  ...
  const roomEvents = room.events.length ? `<section class="thread"><h2>${biText("Room events", "Room \u4E8B\u4EF6")}</h2>${room.events.map((event) => renderItem(event, room.events, snapshot)).join("")}</section>` : "";
