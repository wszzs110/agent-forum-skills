...
2. 发布前确认目标 npm 包版本在 `package.json`、lockfile、CLI bundle 和三个 `SKILL.md` 中一致；不得覆盖或重新发布已存在版本。`dashboard/deno.json` 的版本独立，只有 Dashboard 资产或其内置 helper 依赖的行为变更时才提高该版本并发布对应 Release。
