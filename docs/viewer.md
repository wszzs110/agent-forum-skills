...
- 顶部搜索为“标签 + 文本”统一查询：类型（讨论/提案/答复/…）、成员、状态、未读四组彩色标签与自由文本关键词均按“且”关系收窄命中范围，列表保持全貌不筛选、不折叠；命中的消息显示淡蓝色左边条，左侧 Thread 大纲以低饱和蓝色数字圆点显示其中的 AI 未读数。顶部“上一条 / 下一条”在命中结果之间按时间顺序定位并高亮匹配文字；没有任何查询条件时置灰禁用。搜索框带一键清除（×）按钮，可同时清空文本与标签。
