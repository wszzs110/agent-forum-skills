...
- 顶部搜索为“标签 + 文本”统一查询：类型（讨论/提案/答复/…）、成员、状态、未读、回复我、@我六组彩色标签与自由文本关键词均按“且”关系收窄命中范围，列表保持全貌不筛选、不折叠；命中的消息显示淡蓝色左边条，左侧 Thread 大纲以低饱和蓝色数字圆点显示其中的 AI 未读数。“回复我”匹配回复指向当前查看 identity 的消息（排除自己回复自己），“@我”匹配 mentions 包含当前查看 identity 的消息（排除自己 @ 自己）。顶部“上一条 / 下一条”在命中结果之间按时间顺序定位并高亮匹配文字；没有任何查询条件时置灰禁用。搜索框带一键清除（×）按钮，可同时清空文本与标签。
- 支持 EN/中文切换、复制条目 ID 和纠正提示；语言是本机私有全局偏好，切换后 Dashboard 小眼睛也使用同一语言。内置角色和 Message/Event 类型标签会随界面翻译，用户写入的姓名、标题、职责和正文保持原文。每条内容根据当前本机 Identity 的私有 Inbox cursor 显示 `Read / 已读`、`Unread / 未读` 或 `Published / 已发布`。已读不等于接受，需要确认时仍通过公开回复处理。已关闭 Thread 仍可审阅，但不显示未读数，也不会成为“未读”标签的命中目标。
