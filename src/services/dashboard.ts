...
export async function attachDashboardClient(input: {
  ...
  const client: DashboardClient = { clientId: input.clientId, clientType: input.clientType, forumAlias: forum.alias, forumId: forum.forumId, roomId: input.roomId, identityId: identity.memberId, expiresAt: new Date(Date.now() + leaseMs).toISOString() };
