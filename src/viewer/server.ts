...
function unreadForLocalAi(item: TimelineItem, room: CachedRoom, identities: ViewerReadIdentity[]): boolean {
...
function renderItem(item: TimelineItem, timeline: TimelineItem[], snapshot: ForumSnapshot, room: CachedRoom, identities: ViewerReadIdentity[], index: number, treeIssue?: "missing-parent" | "cycle"): string {
  ...
  const treeIssueNotice = treeIssue
    ...
    : "";
  const unread = unreadForLocalAi(item, room, identities);
  return `<article class="item ${item.kind}" data-timeline-index="${index}"${unread ? " data-ai-unread=\"true\"" : ""}${replyAttributes}>${avatar}<div class="item-main"><div class="item-line"></div><div class="item-content"><header><span class="actor">${escapeHtml(actor)}</span>${profile ? `<span class="role">${localizedLabel(profile.role, "role")}</span>` : ""}<span class="type ${badge}">${localizedLabel(item.type, "type")}</span>${readBadge(item, room, identities)}<time datetime="${escapeHtml(item.createdAt)}">${escapeHtml(formatTime(item.createdAt))}</time></header>${content}${treeIssueNotice}<footer><button class="copy btn-sm" data-copy="${escapeHtml(item.id)}" data-copy-en="${escapeHtml(item.id)}" data-copy-zh="${escapeHtml(item.id)}" data-en="Copy ID" data-zh="复制 ID">Copy ID</button><button class="copy btn-sm" data-copy="${escapeHtml(correctionEn)}" data-copy-en="${escapeHtml(correctionEn)}" data-copy-zh="${escapeHtml(correctionZh)}" data-en="Copy correction prompt" data-zh="复制纠正提示">Copy correction prompt</button></footer></div></div></article>`;
}
...
export function renderViewerHtml(snapshot: ForumSnapshot, room: CachedRoom, freshness: ViewerFreshness = { state: "fresh" }, readIdentities: ViewerReadIdentity[] = [], language: "en" | "zh" = "en"): string {
  ...
  const threadOutlines = room.threads.map((t) => {
    ...
    const unread = t.timeline.filter((item) => unreadForLocalAi(item, room, readIdentities)).length;
    return `<a class="outline-item" href="#thread-${id}" data-title="${title.toLowerCase()}" data-thread-status="${escapeHtml(status)}"><span class="outline-status status-${escapeHtml(status)}" aria-hidden="true"></span><span class="outline-title">${title}</span>${unread ? `<span class="outline-unread" aria-label="${escapeHtml(`${unread} AI unread`)}">${unread}</span>` : ""}<span class="outline-count">${count}</span></a>`;
  }).join("");
...
export async function startViewerServer(input: {
  ...
  let html = renderViewerHtml(currentSnapshot, currentRoom, freshness, readIdentities, language);
    ...
    if (!refreshInFlight) {
      refreshInFlight = input.refresh()
        .then((result) => {
          ...
          html = renderViewerHtml(currentSnapshot, currentRoom, freshness, readIdentities, language);
        })
        .catch(() => {
