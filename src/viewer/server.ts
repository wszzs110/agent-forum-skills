...
function renderItem(item: TimelineItem, timeline: TimelineItem[], snapshot: ForumSnapshot, index: number, treeIssue?: "missing-parent" | "cycle"): string {
...
function renderThread({ thread, timeline }: CachedRoom["threads"][number], snapshot: ForumSnapshot): string {
  ...
  const items = timeline.map((item, index) => renderItem(item, timeline, snapshot, index, item.kind === "message" ? tree.issues.get(item.id) : undefined)).join("");
