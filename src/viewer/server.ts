...
function renderItem(item: TimelineItem, timeline: TimelineItem[], snapshot: ForumSnapshot, room: CachedRoom, identities: ViewerReadIdentity[], index: number, treeIssue?: "missing-parent" | "cycle", trackUnread = true, graphLane?: number): string {
  ...
  let content: string;
  let parent: TimelineItem | undefined = undefined;
  ...
  const repliesToMe = item.kind === "message" && Boolean(item.replyTo) && parent !== undefined && parent.kind === "message" && meIds.has(parent.authorId) && !authoredByMe;
  ...
  const replyAttributes = item.kind === "message"
    ? ` data-message-id="${escapeHtml(item.id)}" data-reply-to="${escapeHtml(item.replyTo ?? "")}"${repliesToMe ? " data-replies-to-me=\"true\"" : ""}${mentionsMe ? " data-mentions-me=\"true\"" : ""}${treeIssue ? ` data-tree-issue="${treeIssue}"` : ""}${graphLane === undefined ? "" : ` data-graph-lane="${graphLane}"`}`
    : "";
