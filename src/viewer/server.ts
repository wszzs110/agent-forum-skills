...
/**
 * 为 Viewer 的 Git Graph 风格树状视图生成稳定的轨道计划。
 * 首个子回复沿用父节点轨道，后续兄弟回复分配新轨道；异常关系继续作为根分支。
 */
export interface ReplyGraphPlan extends ReplyTreePlan {
...
/**
 * 根据回复森林分配可视化轨道，不改变消息的时间线顺序。
 */
export function buildReplyGraph(timeline: TimelineItem[]): ReplyGraphPlan {
  ...
  for (const message of messages) {
    const parentId = parentById.get(message.id);
    const parentLane = parentId === undefined ? undefined : lanes.get(parentId);
    if (parentId === undefined || parentLane === undefined) {
...
/** 当前 Viewer 关联的本机工作区信息，仅在受 token 保护的本机页面展示。 */
export interface ViewerBindingContext {
  workspaceRoot: string;
  branch: string | null;
}

function localizedLabel(value: string, kind: "role" | "type"): string {
  const zh: Record<string, string> = kind === "role"
    ? { backend: "后端", frontend: "前端", fullstack: "全栈", mobile: "移动端", qa: "测试", devops: "运维", security: "安全", designer: "设计", product: "产品" }
    : { discussion: "讨论", question: "问题", answer: "答复", proposal: "提案", decision: "决策", change: "变更", blocker: "阻塞", review: "评审", status: "状态", "test-result": "测试结果", acknowledgement: "确认", objection: "异议", correction: "纠正", "thread-closed": "主题已关闭", "thread-reopened": "主题已重开", "thread-renamed": "主题已重命名" };
  return biText(value, zh[value] ?? value);
}

/** 判断条目是否应作为当前本机身份的未读目标；已关闭 Thread 统一跳过。 */
...
/** 渲染紧凑的本机已读状态；关闭主题不再渲染会吸引处理注意力的未读状态。 */
function readBadge(item: TimelineItem, room: CachedRoom, identities: ViewerReadIdentity[], trackUnread = true): string {
  ...
  if (identities.length === 1) {
    ...
    return read.length
      ? `<span class="read-badge read">${biText("Read", "已读")}</span>`
      : `<span class="read-badge unread">${biText("Unread", "未读")}</span>`;
  }
  const parts: string[] = [];
...
/** 渲染单条消息或事件，并按所属 Thread 的状态决定是否标记本机未读。 */
function renderItem(item: TimelineItem, timeline: TimelineItem[], snapshot: ForumSnapshot, room: CachedRoom, identities: ViewerReadIdentity[], index: number, treeIssue?: "missing-parent" | "cycle", trackUnread = true, graphLane?: number): string {
  const actorId = item.kind === "message" ? item.authorId : item.actorId;
  ...
  const replyAttributes = item.kind === "message"
    ? ` data-message-id="${escapeHtml(item.id)}" data-reply-to="${escapeHtml(item.replyTo ?? "")}"${treeIssue ? ` data-tree-issue="${treeIssue}"` : ""}${graphLane === undefined ? "" : ` data-graph-lane="${graphLane}"`}`
    : "";
...
/** 渲染 Thread 历史；关闭状态仍展示内容，但不提供未读处理提示。 */
function renderThread({ thread, timeline }: CachedRoom["threads"][number], snapshot: ForumSnapshot, room: CachedRoom, identities: ViewerReadIdentity[]): string {
  ...
  const metaZh = `${localizedLabel(kind, "type")} · ${messageCount} 条消息 · ${escapeHtml(creator)} · ${escapeHtml(formatTime(thread.createdAt))}`;
  const graph = buildReplyGraph(timeline);
  const trackUnread = thread.status.toLowerCase() !== "closed";
  const items = timeline.map((item, index) => renderItem(item, timeline, snapshot, room, identities, index, item.kind === "message" ? graph.issues.get(item.id) : undefined, trackUnread, item.kind === "message" ? graph.lanes.get(item.id) : undefined)).join("");
...
export function renderViewerHtml(snapshot: ForumSnapshot, room: CachedRoom, freshness: ViewerFreshness = { state: "fresh" }, readIdentities: ViewerReadIdentity[] = [], language: "en" | "zh" = "en", binding?: ViewerBindingContext, sendMode: "auto" | "ask" = "auto"): string {
  ...
  const roomEvents = room.events.length
    ? `<section class="thread events" id="thread-events" data-title="events"><div class="thread-head"><div class="thread-icon event"></div><div class="thread-meta"><h2>${biText("Events", "事件")}</h2></div></div><div class="thread-body">${room.events.map((event, index) => renderItem(event, room.events, snapshot, room, readIdentities, index)).join("")}</div></section>`
    : "";
  // 当前 Room Viewer 不应被其他 Room 的生命周期状态或损坏历史淹没；兼容旧 cache 时再次去重。
  ...
  const roomArchived = room.room.status.toLowerCase() === "archived"
    ? `<aside class="room-state archived"><strong>${biText("Archived", "已归档")}</strong><span>${biText("This Room is read-only. Return to your Agent conversation to request a restore or correction.", "此 Room 为只读状态。请回到 Agent 会话请求恢复或纠正。")}</span></aside>`
    : "";
  const revision = snapshot.sourceHead;
  const typeZhLabels: Record<string, string> = { discussion: "讨论", question: "问题", answer: "答复", proposal: "提案", decision: "决策", change: "变更", blocker: "阻塞", review: "评审", status: "状态", "test-result": "测试结果", acknowledgement: "确认", objection: "异议", correction: "纠正", "thread-closed": "主题已关闭", "thread-reopened": "主题已重开", "thread-renamed": "主题已重命名" };
  const searchTypeIds: string[] = [];
  const searchMembers: string[] = [];
  const collectSearchable = (item: TimelineItem): void => {
  ...
  const statusLabels: Record<string, [string, string]> = { open: ["Open", "进行中"], active: ["Active", "进行中"], closed: ["Closed", "已关闭"], archived: ["Archived", "已归档"] };
  const searchTagData = {
  ...
  const staleMessageZh = staleMessage === "No remote is configured for this Team; latest remote content cannot be verified."
    ? "此 Forum 未配置 remote，无法验证远端最新内容。"
    : staleMessage === "Remote sync did not complete."
      ? "远端同步未完成。"
      : staleMessage;
  const freshnessNotice = freshness.state === "stale"
    ? `<aside class="sync-state stale"><strong>${biText("Stale", "可能过期")}</strong><span>${biText(staleMessage, staleMessageZh)}</span><button type="button" onclick="location.reload()">${biText("Retry", "重试")}</button></aside>`
    : `<aside class="sync-state fresh"><strong>${biText("Synced", "已同步")}</strong><span>${biText("This page was generated after the latest successful pull-only sync.", "本页面在最近一次成功的只拉取同步后生成。")}</span></aside>`;
  const bindingInfo = binding
    ? `<div class="binding-context"><span>${biText("Path", "目录")}</span><code class="binding-path" title="${escapeHtml(binding.workspaceRoot)}">${escapeHtml(binding.workspaceRoot)}</code><span>${biText("Branch", "分支")}</span><code>${binding.branch === null ? biText("Detached", "游离") : escapeHtml(binding.branch)}</code></div>`
    : "";
...
const searchState={text:'',tags:{type:new Set(),member:new Set(),status:new Set(),unread:false},results:[],selection:null};
const indexes=threads.map(thread=>{
  ...
  thread.querySelectorAll('.item').forEach(item=>{
    ...
    messages.push({item,text,typeId:item.dataset.msgType||'',member:grab('.actor'),status,unread:item.dataset.aiUnread==='true'});
  });
...
function buildTagChips(){
  ...
  const groups=[['type',searchTagData.type||[]],['member',searchTagData.member||[]],['status',searchTagData.status||[]],['unread',searchTagData.unread?[['unread','Unread','未读']]:[]]];
  ...
  groups.forEach(([g,items])=>{
    ...
    items.forEach(item=>{
      ...
      b.addEventListener('click',()=>{
        if(g==='unread'){searchState.tags.unread=!searchState.tags.unread;b.classList.toggle('on',searchState.tags.unread);}
...
function hasQuery(){return Boolean(normQ())||searchState.tags.type.size>0||searchState.tags.member.size>0||searchState.tags.status.size>0||searchState.tags.unread}
function matches(m){
...
function updateSearchClear(){const hasAny=Boolean(search.value)||searchState.tags.type.size>0||searchState.tags.member.size>0||searchState.tags.status.size>0||searchState.tags.unread;if(searchClear)searchClear.classList.toggle('visible',hasAny);}
search.addEventListener('input',()=>{applyQuery();updateSearchClear();});
if(searchClear)searchClear.addEventListener('click',()=>{search.value='';searchState.tags.type.clear();searchState.tags.member.clear();searchState.tags.status.clear();searchState.tags.unread=false;document.querySelectorAll('.tag-chip.on').forEach(c=>c.classList.remove('on'));applyQuery();updateSearchClear();search.focus();});
...
export async function startViewerServer(input: {
  ...
  server = createServer((request, response) => {
    ...
    if (request.method === "POST" && request.url === `${basePath}preferences`) {
      ...
      const value = JSON.parse(body) as { language?: unknown };
      if ((value.language !== "en" && value.language !== "zh") || !input.setLanguage) { response.writeHead(400); response.end("invalid language preference"); return; }
