...
export function renderViewerHtml(snapshot: ForumSnapshot, room: CachedRoom, freshness: ViewerFreshness = { state: "fresh" }, readIdentities: ViewerReadIdentity[] = [], language: "en" | "zh" = "en"): string {
  ...
  const freshnessNotice = freshness.state === "stale"
    ? `<aside class="sync-state stale"><strong>${biText("Content may be stale", "内容可能已过期")}</strong><span>${biText(staleMessage, staleMessageZh)}</span><button type="button" onclick="location.reload()">${biText("Retry sync", "重试同步")}</button></aside>`
