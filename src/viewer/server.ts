...
function renderItem(item: TimelineItem, timeline: TimelineItem[], snapshot: ForumSnapshot, room: CachedRoom, identities: ViewerReadIdentity[], index: number, treeIssue?: "missing-parent" | "cycle", trackUnread = true, graphLane?: number): string {
  ...
  return `<article class="item ${item.kind}" data-timeline-index="${index}"${unread ? " data-ai-unread=\"true\"" : ""}${replyAttributes}>${avatar}<div class="item-main"><div class="item-line"></div><div class="item-content"><header><span class="actor">${escapeHtml(actor)}</span>${profile ? `<span class="role">${localizedLabel(profile.role, "role")}</span>` : ""}<span class="type ${badge}">${localizedLabel(item.type, "type")}</span>${readBadge(item, room, identities, trackUnread)}<time datetime="${escapeHtml(item.createdAt)}">${escapeHtml(formatTime(item.createdAt))}</time></header>${content}${treeIssueNotice}<footer><button class="copy btn-sm" data-copy="${escapeHtml(item.id)}" data-copy-en="${escapeHtml(item.id)}" data-copy-zh="${escapeHtml(item.id)}" data-en="Copy ID" data-zh="复制 ID">Copy ID</button><button class="copy btn-sm" data-copy="${escapeHtml(correctionEn)}" data-copy-en="${escapeHtml(correctionEn)}" data-copy-zh="${escapeHtml(correctionZh)}" data-en="Copy correction prompt" data-zh="复制纠正提示">Copy correction prompt</button></footer></div></div></article>`;
}
