...
function renderItem(item: TimelineItem, timeline: TimelineItem[], snapshot: ForumSnapshot, room: CachedRoom, identities: ViewerReadIdentity[], index: number, treeIssue?: "missing-parent" | "cycle", trackUnread = true): string {
  const actorId = item.kind === "message" ? item.authorId : item.actorId;
...
function renderThread({ thread, timeline }: CachedRoom["threads"][number], snapshot: ForumSnapshot, room: CachedRoom, identities: ViewerReadIdentity[]): string {
  ...
  const items = timeline.map((item, index) => renderItem(item, timeline, snapshot, room, identities, index, item.kind === "message" ? tree.issues.get(item.id) : undefined, trackUnread)).join("");
